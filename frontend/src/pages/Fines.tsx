...
export default function Fines() {
  ...
  const fetchFines = useCallback(async () => {
    setLoading(true);
    try {
      const data = await finesApi.getAll({ search, status });
  ...
  const handlePay = async (id: string) => {
    try { await finesApi.pay(id); fetchFines(); } catch (e) { console.error(e); }
  };
  const handleWaive = async (id: string) => {
    try { await finesApi.waive(id); fetchFines(); } catch (e) { console.error(e); }
  };
  ...
  return (
    <div className="p-8">
      ...
      <Card>
        {loading ? <div className="py-16 text-center text-slate-400 text-sm">Loading fines...</div> : (
          <Table>
                  ...
                  <tr key={fine._id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <Td>{fine.status === "unpaid" && (
                      <div className="flex gap-1">
                        <Button variant="ghost" className="text-xs px-2 py-1 text-emerald-600" onClick={() => handlePay(fine._id)}>Mark Paid</Button>
                        <Button variant="ghost" className="text-xs px-2 py-1 text-slate-500" onClick={() => handleWaive(fine._id)}>Waive</Button>
                      </div>
                    )}</Td>
                  </tr>
                ))
